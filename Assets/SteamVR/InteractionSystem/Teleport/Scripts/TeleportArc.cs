...
namespace Valve.VR.InteractionSystem
{
    ...
    public class TeleportArc : MonoBehaviour
    {
        ...
        [SerializeField]public ArcType currentArcType = ArcType.Parabolic;
        public Vector3 GetArcPositionAtTime(float time)
        {
            if (currentArcType == ArcType.Straight)
            {
                // 直线方程
                return startPos + (projectileVelocity * time);
            }
            else
            {
                // 抛物线方程
                Vector3 gravity = useGravity ? Physics.gravity : Vector3.zero;
                return startPos + ((projectileVelocity * time) + (0.5f * time * time) * gravity) * scale;
            }
        }
